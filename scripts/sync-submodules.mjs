...
//      （pinned），跳过、不报错。
//   6. 不处理嵌套子模块：只同步根 .gitmodules 声明的直接子模块，不递归、不在子模块内提交。
//      子模块内部的嵌套指针由子模块自己的流程负责（职责分离，避免本地 commit 未推送的隐患）。
...
/** 同步根目录 .gitmodules 中声明的所有直接子模块（不处理嵌套） */
function syncDirect(errors) {
    const subs = readSubmodules(ROOT_DIR);
    if (subs.length === 0) return;
    console.log(`\n🔍 根目录：${subs.length} 个子模块`);

    for (const sub of subs) {
        const branch = sub.branch || "";
        if (!ALLOWED_BRANCHES.includes(branch)) {
            console.log(
                `⏭️  [${sub.name}] 未配置 branch 或非 main/master（${branch || "无"}），视为固定 gitlink，跳过`,
            );
            continue;
        }
        if (!sub.path) {
